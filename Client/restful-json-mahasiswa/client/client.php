...
{	private $host;	
	...
	*/
	...
	{	// query ke lokal database client
		...
		foreach ($data as $r)	
		{	// query ke lokal database client
			$query = $this->conn->prepare("insert into mahasiswa (nim,nama,alamat,telepon) values (?,?,?,?)");	
			$query->execute(array($r->nim,$r->nama,$r->alamat,$r->telepon));	
...
$url = 'https://server.pendz-web.my.id/restful-json-mahasiswa/server/server.php';
